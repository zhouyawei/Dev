...
namespace ChatViaSocketClient
{
    public class AsyncClient
    {
        ...
        public void SendTestData()
        {
            ...
            {
                _log.ErrorFormat("AsyncClient->SendTestData出现异常, 当前客户端 = {0}, Exception = {1}", Name, e);
            }
        }
        ...
        private void SendData(Socket clientSocket, byte[] dataInBytes)
        {
        ...
        private void SendDataChunk(Socket clientSocket, byte[] dataInBytes)
        {
            if (clientSocket == null)
            {
                _log.Info(string.Format("AsyncClient->SendDataChunk: clientSocket为空, 当前客户端 = {0}", Name));
                return;
            }

            if (!clientSocket.Connected)
            {
                _log.Info(string.Format("AsyncClient->SendDataChunk: clientSocket已断开与远程主机的连接, 当前客户端 = {0}", Name));
                return;
            }
        ...
        private void ReceiveAsync(Socket clientSocket)
        {
        ...
        /*抽象数据处理方法，receivedBytes是一个完整的包*/
        protected void ProcessData(AsyncUserToken asyncUserToken, byte[] receivedBytes)
        {
            string message = Encoding.UTF8.GetString(receivedBytes, 0, receivedBytes.Length);

            Console.WriteLine(message);

            _log.Info(string.Format("ProcessData: 当前客户端 = {0}, message = {1}", Name, message));
        }
...
　　注：文中部分案例来自资深游戏猎头 Vivi 与豆丁";
        ...
        private List<AsyncUserToken> _asyncUserTokenPool = new List<AsyncUserToken>();
